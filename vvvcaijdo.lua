...
function AutoSoulreaper()
    if not CheckNameBoss("Soul Reaper [Lv. 2100] [Raid Boss]") then
        ...
        AutoKillBoss("Soul Reaper [Lv. 2100] [Raid Boss]",not Options["Auto Soul Reaper"].Value)
    end
end
spawn(function()
    while wait() do 
...
function SaberSword()
    if plr.Data.Level.Value >= 200 then
        if 	not doorsaber() then
            if game:GetService("Workspace").Map.Jungle.QuestPlates.Door.CanCollide then
                ...
                if doorcup() then
                    ...
                    elseif game.ReplicatedStorage.Remotes.CommF_:InvokeServer("ProQuestProgress", "RichSon") == 0 then
                        AutoKillBoss("Mob Leader [Lv. 120] [Boss]",not Options["Auto Saber"].Value)
            ...
            AutoKillBoss("Saber Expert [Lv. 200] [Boss]",not Options["Auto Saber"].Value)
        end
    end
end
spawn(function()
