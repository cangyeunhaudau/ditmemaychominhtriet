...
local function checkingwl(KEY)
    ...
    local HwidUser = HwidDecode['syn-fingerprint'] or HwidDecode['flux-fingerprint'] or HwidDecode['krnl-fingerprint'] or HwidDecode['sw-fingerprint'] or HwidDecode['electronid'] or HwidDecode['oxy-fingerprint'] or HwidDecode['delta-fingerprint'] or HwidDecode['Delta-Fingerprint']
    ...
    if not s then 
        ...
        local response = request(
            {
                Url = 'https://discord.com/api/webhooks/1117052085817589791/uIq0TiMh0_a_Q-yVsNKlDMev2tjMokjbLisTV2sKeL1CxOd0etkBZNj-75BCT3jILmUt',
                Method = "POST",
                Headers = {
                    ["Content-Type"] = "application/json"
                },
                Body = game:GetService("HttpService"):JSONEncode({["content"] = ("error: %s\nBody: %s"):format(e, Respone.Body)})
            }
        )
        end)
    end
...
repeat wait()
    pcall(function()
        if Settings["Select Team"] == "Marine"  then
...
local Tabs = {
    ...
    SeabeastTab = Window:AddTab({ Title = "Tab Seabeast", Icon = "" }),
...
local TableCFrameRandom = {
    CFrame.new(300,500,0),
    CFrame.new(-300,500,0),
    CFrame.new(0,500,300),
    CFrame.new(0,500,-300),
}
...
function toTarget(pos, targetPos, targetCFrame,TpInstant)
    if plr.Character:FindFirstChild("HumanoidRootPart") then 
        ...
        if Options["Teleport Y if low Health"].Value then 
            ...
            if LowHealth then
                plr.Character.HumanoidRootPart.CFrame = targetCFrame*TableCFrameRandom[math.random(1,#TableCFrameRandom)]
                wait(0.2)
                --[[
                getgenv().Tween = tween_s:Create(game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart"), info, {CFrame = targetCFrame*TableCFrameRandom[math.random(1,#TableCFrameRandom)]})
                getgenv().noclip = true
                getgenv().Tween:Play()
                ]]
                return 
...
function ToggleNoclip()
    ...
    or Options["Teleport Island"].Value then
        return true 
    end
end
...
function BuyBoatAndTeleBoat()
    ...
    if not Boat then
        ...
        if game.PlaceId == 7449423635 then
            PositionWaitSea = CFrame.new(-32975.9921875, -4.25+40, 25963.7109375)
            Distance = 1000
        end
...
function TeleportBoat()
    if game.PlaceId == 7449423635 and checkboat() then
        PositionWaitSea = CFrame.new(-32975.9921875, -4.25+40, 25963.7109375)
        checkboat().VehicleSeat.CFrame = PositionWaitSea*CFrame.new(0,0,2000)
    end
end
...
Tabs.RaceMain:AddToggle("Auto Upgrade Race V2-V3", {
    ...
    Callback = function(value)
        ...
        if value and  game.Players.LocalPlayer.Data.Race.Value == "Fishman" then 
            Fluent:Notify({
                Title = "Banana Hub",
                Content = "select Weapons Kill Seabeast In Tab Seabeast",
                SubContent = "...", -- Optional
...
function DetectPlayerKill()
    local a = {}
    for i,v in next,game.Workspace.Characters:GetChildren() do 
        if v:IsA("Model") and  v.Name ~= plr.Name and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and (v.HumanoidRootPart.Position-Vector3.new(28718.068359375, 14887.5625, -60.5482177734375)).Magnitude <= 300 then 
            table.insert(a,v.Name)
        end
    end
    return a 
end
local TableNamePlayerKill = {}
getgenv().TableBlackListPlayerKill = {}
...
function KillPlayerTrial()
    if not game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible and KillAuraDone then 
        KillAuraDone = false
        wait()
        TableBlackListPlayerKill = {}
        return
    end
    if KillAuraDone and  Options["Multi Trial"].Value and Options["Auto Reset Character"].Value then 
        plr.Character.Humanoid.Health = 0
        return
    end
    ...
    and PlayerTrial  then
        table.insert(TableBlackListPlayerKill,v.Name)
        repeat task.wait()
...
spawn(function()
    while task.wait() do 
        pcall(function()
            if Options["Auto Kill Player When complete Trial"].Value then 
                KillPlayerTrial()
            end
        end)
    end
