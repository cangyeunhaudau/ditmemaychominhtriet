...
if not Fluxus then 
    return game.Players.LocalPlayer:Kick("Use Fluxus")
end
...
if not (#getgenv().Key ~= 16 or #getgenv().Key ~= 24) then 
...
local Tabs = {
    ...
    SettingTab = Window:AddTab({ Title = "Tab Setting", Icon = "" }),
...
Tabs.FarmMain:AddToggle("Auto Turn On V3", {
    Title = "Auto Turn On  V3",
    Default = Settings["Auto Turn On V3"] or false,
    Callback = function(value)
        SaveSettings("Auto Turn On V3",value)
    end
})
...
function toTarget(pos, targetPos, targetCFrame,TpInstant)
    if plr.Character:FindFirstChild("HumanoidRootPart") then 
        ...
        if plr:DistanceFromCharacter(targetPos) < 1500 and not TpInstant then 
            aaa = 170
        end
...
function ToggleNoclip()
    ...
    or Options["Teleport Mirage"].Value
    or Options["Auto Soul Reaper"].Value
    or Options["Auto Saber"].Value then
        return true 
    end
end
...
function DetectMob(c)
    ...
    for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
        if ((typeof(c) == "table" and table.find(c,v.Name)) or v.Name == c) and IsMobAlive(v) then
...
function DetectAttack(v)
    if plr.Character:FindFirstChild("HumanoidRootPart") and (plr.Character.HumanoidRootPart.Position-v.HumanoidRootPart.Position).Magnitude < 50 then
        ...
        repeat task.wait() until tick()-timedelay >= Options["Select Speed Attack"].Value
    end
end
--------Farm Mastery--------
    ...
    if Options["Auto Mastery"].Value then
        ...
        if v.Humanoid.Health / v.Humanoid.MaxHealth > Options["Health %"].Value/100  then 
            ...
            if condimebeo then 
                ...
                task.wait(Options["SKill "..condimebeo.." Mastery"].Value)
...
function CheckNameBoss(a)
    for i,v in next,game.ReplicatedStorage:GetChildren() do
        if ((typeof(a) == "table" and table.find(a,v.Name)) or v.Name == a) and IsMobAlive(v) then
            return v
        end
    end
...
CameraShakerR:Stop()
function StopFarm()
    if Options["Teleport To Fruit"].Value and GetPathFruit() then StartFarm = false return  end
...
function FarmMethod()
    ...
    if game.PlaceId == 2753915549 and  plr.Data.Level.Value < 120 and plr.Data.Level.Value >= 10 then
        MobLevelFarm = {"Royal Squad","Shanda"}
    end
    if not plr.PlayerGui.Main:FindFirstChild("Quest").Visible and typeof(MobLevelFarm) ==  "string"  then
        ...
        if  Options["Select Method Farm"].Value == "Farm Katakuri"  then
            if CheckNameBoss("Cake Prince")  then 
                AutoKillBoss("Cake Prince", (not StartFarm or not Options["Auto Farm"].Value))
                return 
            else
    ...
    if game.PlaceId == 2753915549 and plr.Data.Level.Value >= 700 then
        if game.ReplicatedStorage.Remotes.CommF_:InvokeServer("DressrosaQuestProgress", "Dressrosa") ~= 0 then
            if game.Workspace.Map.Ice.Door.CanCollide then
                ...
                if CheckNameBoss("Ice Admiral")  then 
                    AutoKillBoss("Ice Admiral", not Options["Auto Quest New World"].Value)
                end
            end
        else
...
spawn(function()
    while  task.wait() do
        ...
        if Options["Auto Turn On V3"].Value then 
...
Tabs.MiscMain:AddToggle("Auto Soul Reaper", {
    Title = "Kill Soul Reaper",
    Default = Settings["Kill Soul Reaper"] or false,
    Callback = function(value)
        SaveSettings("Kill Soul Reaper",value)
    end
})
------------------Event Tab------------------
function DetectCore()
    for i,v in next,game.ReplicatedStorage:GetChildren() do
        if v:IsA("Model") and   v.Name == "Core" and v:FindFirstChild("HumanoidRootPart") then
            return v
        end
    end
    for i,v in next,game.Workspace.Enemies:GetChildren() do
...
function EventFactory()
    getgenv().DetectFactory = DetectCore()
    if DetectFactory then
        local v = DetectFactory
        repeat task.wait()
            if plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.Health == 0 then 
                return 
            end
            ...
            DetectAttack(v)
            equiptool(NameWeapon(Options["Select Weapon"].Value))
        until not v or not v.Parent  or not Options["Auto Factory"].Value
    end
end
...
function AutoEliteHunter()
    ...
    if not string.find(game.Players.LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text,v.Name) 
...
spawn(function()
    while wait() do 
        pcall(function()
            if Options["Kill Rip Indra"].Value and CheckNameBoss("rip_indra True Form") then 
                AutoKillBoss("rip_indra True Form",not Options["Kill Rip Indra"].Value)
...
function DoughKing()
    if  CheckNameBoss("Dough King") then
        AutoKillBoss("Dough King",not Options["Auto Dough King"].Value)
...
spawn(function()
    while wait() do 
        pcall(function()
            if Options["Kill DarkBeard"].Value and CheckNameBoss("Darkbeard") then
                AutoKillBoss("Darkbeard", not Options["Kill DarkBeard"].Value)
...
function FarmObservation()
    ...
    if not game:GetService("Lighting").Blur.Enabled then
        ...
        game:GetService("VirtualInputManager"):SendKeyEvent(false, "E", false, game)
        task.wait(3)
        if not game:GetService("Lighting").Blur.Enabled and Options["Hop Server Farm Observation"].Value then 
            HopServer()
        end
    else
        if not mob then
            ...
            until not Options["Farm Observation"].Value or not game:GetService("Lighting").Blur.Enabled
        end
    end
end
spawn(function()
...
function AutoSoulreaper()
    if not CheckNameBoss("Soul Reaper") then
        if not DetectItemPlr("Hallow Essence")  then
            spawn(function()
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bones","Buy",1,1)
            end)
            AutoKillMob(TableMobBones,not Options["Auto Soul Reaper"].Value)
        else
        ...
        AutoKillBoss("Soul Reaper",not Options["Auto Soul Reaper"].Value)
    end
end
spawn(function()
    while wait() do 
        pcall(function()
            if Options["Auto Soul Reaper"].Value  then
                AutoSoulreaper()()
            end
        end)
    end
...
local ZoneSeabeast = {
    ["Zone 1"]  = CFrame.new(-21767.4765625, -4.25+50, 5815.41259765625),
    ["Zone 2"]  = CFrame.new(-26017.931640625, -4.25+50, 5657.8837890625),
    ["Zone 3"]  = CFrame.new(-29545.703125, -4.25+50, 6377.98974609375),
    ["Zone 4"]  = CFrame.new(-33609.7578125, -4.25+50, 7422.890625),
    ["Zone 5"]  = CFrame.new(-38480.42578125, -4.25+50, 10350.943359375),
    ["Zone 6"]  = CFrame.new(-32975.9921875, -4.25+50, 25963.7109375)
}
local TableBoat = {
    "Beast Hunter",
    "Guardian",
...
Tabs.SeabeastTab:AddDropdown("Select Boat", {
    Title = "Select Boat",
    Values = TableBoat,
    Multi = false,
    Default = Settings["Select Boat"] or "GrandBrigade",
    Callback = function(value)
        SaveSettings("Select Boat",value)
    end
})
...
Tabs.SeabeastTab:AddDropdown("Select Friend", {
    Title = "Select Friend",
    Values = DetectNamePlayer(),
    Multi = false,
    Default = Settings["Select Friend"] or "",
    Callback = function(value)
        SaveSettings("Select Friend",value)
    end
})
Tabs.SeabeastTab:AddButton({
    ...
    Description = "",
    Callback = function()
        Options["Select Friend"]:SetValues(DetectNamePlayer())
    end
})
Tabs.SeabeastTab:AddToggle("Auto Sea Event With Friend", {
    Title = "Auto Sea Event With Friend",
    Default = Settings["Auto Sea Event With Friend"] or false,
    Callback = function(value)
        SaveSettings("Auto Sea Event With Friend",value)
    end
})
...
Tabs.SeabeastTab:AddSlider("Teleport Y Attack Terrorshark", {
    ...
    Description = "",
    Default = Settings["Teleport Y Attack Terrorshark"] or 60,
    Min = 50,
    Max = 80,
    Rounding = 1,
    Callback = function(value)
...
function UseSkillSeabeast(x)
    ...
    if string.find(x.Name,"Leviathan") then 
        Distance = 2000
    end
    ...
    if plr:DistanceFromCharacter(namexczxc) <= Distance  then 
        ...
        if NameSkill then
            ...
            local HoldSkill = (plr.Character:FindFirstChild(NameWpSeabeast1) and "1") or (plr.Character:FindFirstChild(NameWpSeabeast2) and "2")  
            game:GetService("VirtualInputManager"):SendKeyEvent(true, NameSkill.Name, false, game)
            task.wait(Options["SKill "..NameSkill.Name.." Weapon "..HoldSkill].Value)
...
------------------Buy Boat------------------
function checkboat()
    local NamePlayer = plr.Name 
    if Options["Auto Sea Event With Friend"].Value and Options["Auto Sea Event"].Value then 
        NamePlayer = Options["Select Friend"].Value
    end
    for i,v in next,game:GetService("Workspace").Boats:GetChildren() do
        if v:IsA("Model") then
            if v:FindFirstChild("Owner") and  tostring(v.Owner.Value) == NamePlayer and v.Humanoid.Value > 0 then
                return v
            end
        end
    end
...
function CheckSeatBoatFriend()
    for i,v in next,checkboat():GetChildren() do
        if v.Name == "Cannon" and not v.Seat:FindFirstChild("SeatWeld") then 
            return v
        end
    end 
end
function BuyBoatAndTeleBoat()
    ...
    if Options["Auto Sea Event With Friend"].Value then
        if Boat then
            local BoatFriend = CheckSeatBoatFriend()
            if not plr.Character.Humanoid.Sit then
                if BoatFriend then 
                    toTarget(plr.Character.HumanoidRootPart.Position,BoatFriend.Seat.Position,BoatFriend.Seat.CFrame)
                else
                    toTarget(plr.Character.HumanoidRootPart.Position,Boat.Engine.Position,Boat.Engine.CFrame)
                end
            end
        end
        return 
    ...
    if not Boat then
        ...
        if (PositionGetBoat.Position-plr.Character.HumanoidRootPart.Position).Magnitude > 8 then
            ...
            local SelectBoat = Options["Select Boat"].Value 
            if SelectBoat == "Brigade" or SelectBoat == "GrandBrigade" then 
                SelectBoat = "Pirate"..Options["Select Boat"].Value
            end
...
function AutoSeabeast()
    ...
    if not v then 
        ...
        if TeleportYIngore(v) and not Options["Auto Sea Event With Friend"].Value  then
            local time = tick()
            ...
            until tick()-time >= 3 or not Options["Auto Sea Event"].Value 
            return 
        end
        if Options["Auto Sea Event With Friend"].Value and plr:DistanceFromCharacter(checkboat().VehicleSeat.Position) > 5000 then
            toTarget(plr.Character.HumanoidRootPart.Position,checkboat().Engine.Position,checkboat().Engine.CFrame)
            return
        end
        repeat task.wait()
            TeleportBoat()
...
local mobilefly = function(speaker, vfly)
    ...
    ScreenGuiFly.Name = "ScreenGuiFly"
    ...
    FlyUp.Name = "FlyUp"
    FlyUp.Parent = ScreenGuiFly
    FlyUp.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    FlyUp.BackgroundTransparency = 1.000
    FlyUp.BorderColor3 = Color3.fromRGB(0, 0, 0)
    FlyUp.BorderSizePixel = 0
    FlyUp.Position = UDim2.new(0.158661261, 0, 0.82663101, 0)
    FlyUp.Size = UDim2.new(0.0538219661, 0, 0.0765434727, 0)
    FlyUp.Font = Enum.Font.SourceSans
    FlyUp.Text = "↑"
    FlyUp.TextColor3 = Color3.fromRGB(0, 0, 0)
    FlyUp.TextScaled = true
    FlyUp.TextSize = 14.000
    FlyUp.TextWrapped = true
    
    FlyDown.Name = "FlyDown"
    FlyDown.Parent = ScreenGuiFly
    FlyDown.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    FlyDown.BackgroundTransparency = 1.000
    FlyDown.BorderColor3 = Color3.fromRGB(0, 0, 0)
    FlyDown.BorderSizePixel = 0
    FlyDown.Position = UDim2.new(0.158661261, 0, 0.922887683, 0)
    FlyDown.Size = UDim2.new(0.0538219661, 0, 0.0765434727, 0)
    FlyDown.Font = Enum.Font.SourceSans
    FlyDown.Text = "↓"
    FlyDown.TextColor3 = Color3.fromRGB(0, 0, 0)
    FlyDown.TextScaled = true
    FlyDown.TextSize = 14.000
    FlyDown.TextWrapped = true
	mfly2 = game:GetService('RunService').RenderStepped:Connect(function()
		...
		if speaker.Character:FindFirstChildWhichIsA("Humanoid") and root and root:FindFirstChild(velocityHandlerName)  then
            ...
            FlyDown.MouseButton1Click:Connect(function()
                VelocityHandler.Velocity = Vector3.new(0,-20,0)
            end)
...
Tabs.DFRaidMain:AddToggle("Hop Server [Teleport To Fruit]", {
    Title = "Hop Server [Teleport To Fruit]",
    Default = Settings["Hop Server [Teleport To Fruit]"] or false,
    Callback = function(value)
        SaveSettings("Hop Server [Teleport To Fruit]",value)
    end
})
Tabs.DFRaidMain:AddParagraph({
    Title = "Raids",
...
function TeleportToFruit()
    if Options["Auto Quest New World"].Value and  game.PlaceId == 2753915549 and plr.Data.Level.Value >= 700 then return  end
    if Options["Auto Factory"].Value and  getgenv().DetectFactory then  return  end 
    if Options["Auto Raid Castle"].Value and getgenv().DetectRaidCastle then  return  end 
    if Options["Auto Elite Hunter"].Value and  getgenv().DetectElite then  return  end 
    ...
    if Fruit then
        ...
        if Options["Hop Server [Teleport To Fruit]"].Value then 
            HopServer()
        end
    end
end
    ...
    if Race == " V1" then 
        ...
        elseif game.ReplicatedStorage.Remotes.CommF_:InvokeServer("Alchemist", "1") == 1 then
            ...
            elseif not DetectItemPlr("Flower 3") then 
                AutoKillMob("Swan Pirate", not Options["Auto Upgrade Race V2-V3"].Value)
            end
        ...
        if CheckRaceV2 == "Human V2" then 
            local DetectbossHuman =  (not table.find(BlBossHuman,"Jeremy") and CheckNameBoss("Jeremy")) or (not table.find(BlBossHuman,"Fajita") and CheckNameBoss("Fajita")) or (not table.find(BlBossHuman,"Diamond") and CheckNameBoss("Diamond"))
...
function PullLeverV4()
    if not  CheckItemInventory("Valkyrie Helm") or not CheckItemInventory("Mirror Fractal") then 
        Fluent:Notify({
            Title = "Banana Hub",
            Content = "Not Valkyrie Helm or not Mirror Fractal ",
            SubContent = "...", -- Optional
            Duration = 5 -- Set to nil to make the notification not disappear
        })
        wait(5)
    ...
    if game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible then
        ...
        elseif Race == "Fishman" then
            ...
            if FishmanTrial and (FishmanTrial.Position-plr.Character.HumanoidRootPart.Position).Magnitude < 1500 then
                ...
                until not v or not v.Parent or v.Health.Value == 0
            end
        ...
        if not getgenv().TrialDone and not game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible   then 
            ...
            for i,v in pairs(plr.PlayerGui.Notifications:GetDescendants()) do
                if v:IsA("TextLabel") then 
                    if string.find(v.Text,"completed your trial!")  or string.find(v.Text,"All trials have been completed!") then 
                        getgenv().TrialDone = true 
                    end
                    if string.find(v.Text,"FIGHT!") then
                        task.wait(1)
                        getgenv().KillAuraDone = true 
                    end
                end
            end
        end)
...
function KillPlayerTrial()
    if not game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible and getgenv().KillAuraDone then 
        getgenv().KillAuraDone = false
        return
    end
    if getgenv().KillAuraDone  and Options["Auto Reset Character"].Value then 
        plr.Character.Humanoid.Health = 0
        return
    end
    local PlayerTrial = DetectPlayerKillauraTrial()
    if getgenv().KillAuraDone 
...
spawn(function()
    while task.wait() do 
        pcall(function()
            if Options["Auto Kill Player When complete Trial"].Value or Options["Auto Reset Character"].Value then 
                KillPlayerTrial()
            end
        end)
    end
...
local Statusspy = Tabs.StatusSvMain:AddParagraph({
    Title = "Status SPY",
    Content = "",
})
...
spawn(function()
    while wait() do 
        pcall(function()
            ...
            Statusspy:SetDesc(StatusCheckLeviathan())
...
Tabs.ItemsMain:AddToggle("Auto Saber", {
    Title = "Auto Saber",
        ...
        if not CommF:InvokeServer("GuitarPuzzleProgress", "Check").Swamp then
            if (CFrame.new(-10171.7607421875, 138.62667846679688, 6008.0654296875).Position-plr.Character.HumanoidRootPart.Position).Magnitude > 100 then
                ...
                if CountZombie() == 6 then  
                    AutoKillMob("Living Zombie",not Options["Auto Soul Guitar"].Value)
                end
            end
            return
    ...
    if not checkcountitem("Dark Fragment",1)  then
        if game.PlaceId == 4442272183 then
            if CheckNameBoss("Darkbeard") then 
                AutoKillBoss("Darkbeard", not Options["Auto Soul Guitar"].Value)
            else
...
function Questgood5()
    if (game:GetService("Workspace")["_WorldOrigin"].Locations["Heavenly Dimension"].Position-plr.Character.HumanoidRootPart.Position).Magnitude < 1000 then
        ...
        if CheckNameBoss("Cake Queen")  then
            AutoKillBoss("Cake Queen",not Options["Auto CDK"].Value)
            if getgenv().Tween then
                getgenv().Tween:Pause()
                getgenv().Tween:Cancel()
            end
...
function QuestEvil5()
    if (game:GetService("Workspace")["_WorldOrigin"].Locations["Hell Dimension"].Position-plr.Character.HumanoidRootPart.Position).Magnitude > 1000 then
        if not CheckNameBoss("Soul Reaper") then
            ...
            local v = CheckNameBoss("Soul Reaper")
            repeat task.wait()
    ...
    if game.ReplicatedStorage.Remotes.CommF_:InvokeServer("EliteHunter", "Progress") < 30 then 
        ...
        if (game.Workspace.Map.Waterfall.SealedKatana.Handle.Position-plr.Character.HumanoidRootPart.Position).Magnitude > 50 then
            ...
            if game.Workspace.Enemies:FindFirstChild("Ghost") then
                AutoKillMob("Ghost",not Options["Auto Yama"].Value)
            else
...
function GetTushita()
    if not game:GetService("Workspace").Map.Turtle:FindFirstChild("TushitaGate") then
        if CheckNameBoss("Longma") then 
            AutoKillBoss("Longma",not Options["Auto Tushita"].Value)
        end
    else
        if CheckNameBoss("rip_indra True Form") then
...
function SaberSword()
    if plr.Data.Level.Value >= 200 then
        if 	not doorsaber() then
            if game:GetService("Workspace").Map.Jungle.QuestPlates.Door.CanCollide then
                ...
                if doorcup() then
                    ...
                    elseif game.ReplicatedStorage.Remotes.CommF_:InvokeServer("ProQuestProgress", "RichSon") == 0 then
                        AutoKillBoss("Mob Leader",not Options["Auto Saber"].Value)
            ...
            AutoKillBoss("Saber Expert",not Options["Auto Saber"].Value)
        end
    end
end
spawn(function()
...
spawn(function()
    while task.wait() do
        if Options["Auto Saber"].Value then
            pcall(function()
                SaberSword()
            end)
        end
    ...
    while wait() do 
        if Options["Auto Kill All Boss"].Value then 
            pcall(function()
                ...
                AutoKillBoss(TableListBoss,not Options["Auto Kill All Boss"].Value)   
            end)
        end
    end
end)
...
------------------Done tab pvp------------------
------------------dang lam  tab Setting------------------\
Tabs.SettingTab:AddParagraph({
    Title = "Hold Skill Farm Mastery",
    Content = "....",
})
Tabs.SettingTab:AddInput("SKill Z Mastery", {
    Title = "SKill Z Mastery",
    Default = Settings["SKill Z Mastery"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill Z Mastery",value)
    end
})
Tabs.SettingTab:AddInput("SKill X Mastery", {
    Title = "SKill X Mastery",
    Default = Settings["SKill X Mastery"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill X Mastery",value)
    end
})
Tabs.SettingTab:AddInput("SKill C Mastery", {
    Title = "SKill C Mastery",
    Default = Settings["SKill C Mastery"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill C Mastery",value)
    end
})
Tabs.SettingTab:AddInput("SKill V Mastery", {
    Title = "SKill V Mastery",
    Default = Settings["SKill V Mastery"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill V Mastery",value)
    end
})
Tabs.SettingTab:AddParagraph({
    Title = "Hold Skill Tab Sea Event",
    Content = "....",
})
Tabs.SettingTab:AddInput("SKill Z Weapon 1", {
    Title = "SKill Z Weapon 1",
    Default = Settings["SKill Z Weapon 1"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill Z Weapon 1",value)
    end
})
Tabs.SettingTab:AddInput("SKill X Weapon 1", {
    Title = "SKill X Weapon 1",
    Default = Settings["SKill X Weapon 1"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill X Weapon 1",value)
    end
})
Tabs.SettingTab:AddInput("SKill C Weapon 1", {
    Title = "SKill C Weapon 1",
    Default = Settings["SKill C Weapon 1"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill C Weapon 1",value)
    end
})
Tabs.SettingTab:AddInput("SKill V Weapon 1", {
    Title = "SKill V Weapon 1",
    Default = Settings["SKill V Weapon 1"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill V Weapon 1",value)
    end
})
Tabs.SettingTab:AddInput("SKill F Weapon 1", {
    Title = "SKill F Weapon 1",
    Default = Settings["SKill F Weapon 1"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill F Weapon 1",value)
    end
})
Tabs.SettingTab:AddInput("SKill Z Weapon 2", {
    Title = "SKill Z Weapon 2",
    Default = Settings["SKill Z Weapon 2"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill Z Weapon 2",value)
    end
})
Tabs.SettingTab:AddInput("SKill X Weapon 2", {
    Title = "SKill X Weapon 2",
    Default = Settings["SKill X Weapon 2"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill X Weapon 2",value)
    end
})
Tabs.SettingTab:AddInput("SKill C Weapon 2", {
    Title = "SKill C Weapon 2",
    Default = Settings["SKill C Weapon 2"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill C Weapon 2",value)
    end
})
Tabs.SettingTab:AddInput("SKill V Weapon 2", {
    Title = "SKill V Weapon 2",
    Default = Settings["SKill V Weapon 2"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill V Weapon 2",value)
    end
})
Tabs.SettingTab:AddInput("SKill F Weapon 2", {
    Title = "SKill F Weapon 2",
    Default = Settings["SKill F Weapon 2"] or 0.5,
    Numeric = true, -- Only allows numbers
    Finished = false, -- Only calls callback when you press enter
    Callback = function(value)
        SaveSettings("SKill F Weapon 2",value)
    end
})
